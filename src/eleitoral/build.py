...
from . import (config, comparecimento, contas, crosswalk, eleitorado,
...
def main(offline: bool = False) -> None:
    ...
    # Emendas parlamentares FEDERAIS: NÃO entram aqui. São processadas à parte por
    # `python -m eleitoral.emendas_federal` (download em massa do Portal da
    # Transparência, manual no navegador por causa do WAF), que gera
    # docs/data/emendas_resumo.json + docs/data/emendas/{uf}.json e carimba
    # meta.emendas_gerado_em. O front carrega esses sob demanda.
    ...
    for ind in inds:
        ...
        # Emendas federais: processadas à parte (eleitoral.emendas_federal).
        registros.append(d)
...
def _escrever_meta(manifest: Manifest, ano_pop: str, dt_eleitorado: str) -> None:
    # preserva o carimbo das emendas federais (gerado à parte por emendas_federal),
    # senão o front perde a versão de cache do emendas_resumo/{uf}.json.
    emendas_gerado_em = None
    if config.DOCS_DATA.joinpath("meta.json").exists():
        try:
            emendas_gerado_em = json.loads(
                (config.DOCS_DATA / "meta.json").read_text(encoding="utf-8")
            ).get("emendas_gerado_em")
    ...
    meta = {
        ...
        "indicadores": {
            ...
            "emendas_federais": {
                "definicao": (
                    "emendas parlamentares FEDERAIS de TODAS as modalidades "
                    "(individual, bancada, comissão, relator) recebidas pelo "
                    "município (2020 em diante), com modalidade, área, autor e casa, "
                    "e valor por mês (filtro de período no front)."
                ),
                "metodo": (
                    "Download em massa 'Emendas por Documentos de Despesa' do Portal "
                    "da Transparência (CGU), um CSV por ano, baixado manualmente no "
                    "navegador (o WAF bloqueia automação). Atribuído ao MUNICÍPIO DE "
                    "APLICAÇÃO (código IBGE oficial do Portal). Processado por "
                    "eleitoral.emendas_federal → docs/data/emendas_resumo.json + "
                    "emendas/{uf}.json. Atualização semanal."
                ),
                "ressalva": config.NOTA_EMENDAS,
                "fonte": "Controladoria-Geral da União — Portal da Transparência; Câmara e Senado (casa)",
            },
            "votos_branco_nulo": {
